...
export class Bullet extends Entity {
  ...
  init(x: number, y: number, target: Monster, hurt: number, uid: number) {
    ...
    let dx = Math.floor(this.target.node.x - this.node.x);
    let dy = Math.floor(0 - this.node.y);
    let angle = f_Math.atan2(dy, dx);
    let delta = new Decimal(this.speed).div(kScale);
    this.node.getComponent(cc.RigidBody).linearVelocity = new cc.Vec2(delta.mul(f_Math.cos(angle)).floor(), delta.mul(f_Math.sin(angle)).floor())
  }
