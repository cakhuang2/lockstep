...
export class Game extends cc.Component {
  ...
  onLoad() {
    ...
    if (!cc.director.getPhysicsManager().enabled) {
      ...
      Math.random = () => {
        return util.f_Math.random().toNumber()
      }
      cc.director.getScheduler().unscheduleUpdate(cc.director.getPhysicsManager())
      cc.director.getPhysicsManager().enabled = true
      cc.director.getPhysicsManager()["update"] = function (dt) {
  ...
  update(dt) {
    network.readMsg();
    // this.readTime -= dt;
    // if ((this.readTime <= 0 && PlayerInfo.isMovie) || (!PlayerInfo.isMovie && this.physicalTime < physicalDt)) {
    //   this.readTime = this.readPerTime + this.readTime;
    this._readFrame();
    // }
    if (this.physicalTime >= physicalDt) {
      this.physicalTime -= physicalDt
      cc.director.getPhysicsManager()["update"]()
    }

    // !this.gameOver && this.monsters[0] && console.log(this.monsters[0].node.position.x, this.monsters[0].node.position.y)
    // !this.gameOver && this.bullets[0] && console.log("bullet", this.bullets[0].node.position.x, this.bullets[0].node.position.y)
    if (!this.printed && !this.gameOver && this.monsters[0] && this.monsters[0].node.position.y !== 0) {
      this.printed = true
      console.log(this.monsters[0].node.position.y)
    }
  }
  ...
  private _newMonster() {
    ...
    // this.scheduleOnce(com.die.bind(com), 5)
  }
