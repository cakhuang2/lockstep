...
export class Monster extends Entity {
  ...
  init() {
    ...
    this.node.getComponent(cc.RigidBody).linearVelocity = new cc.Vec2(30, 0)
    this.node.getComponent(cc.RigidBody).enabledContactListener = true
    // this.node.getComponent(cc.RigidBody).onBeginContact = this.onBeginContact
  }

  onBeginContact(contact: cc.PhysicsContact, selfCollider: cc.PhysicsCollider, otherCollider: cc.PhysicsCollider) {
    console.log(selfCollider.node.position.toString(), otherCollider.node.position.y)
  }
